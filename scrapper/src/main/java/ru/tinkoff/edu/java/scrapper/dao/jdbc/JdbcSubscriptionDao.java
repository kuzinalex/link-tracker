...
public class JdbcSubscriptionDao implements SubscriptionDao {
    ...
    private static final String SQL_INSERT_SUBSCRIPTION = "INSERT INTO subscription  VALUES (?,?)";
    private static final String SQL_REMOVE_SUBSCRIPTION = "DELETE FROM subscription WHERE chat_id=(?) AND link_id=(?)";
    private static final String SQL_FIND_SUBSCRIPTION = "SELECT * FROM subscription WHERE chat_id=(?) AND link_id=(?)";
